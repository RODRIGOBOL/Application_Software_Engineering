...
const Playlist: React.FC<PlaylistProps> = ({ tracks }) => {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* --- SECTION HERO : TITRE EN COURS + LECTEUR PRINCIPAL --- */}
      <div className="bg-gradient-to-r from-blue-900 to-indigo-900 rounded-xl p-6 mb-6 shadow-xl relative overflow-hidden flex-shrink-0">
        ...
        {/* Infos contextuelles (Gardées pour le style) */}
        <div className="flex items-start gap-6 relative z-10 mb-4">
          <img 
            src={currentTrack.coverUrl} 
            alt={currentTrack.name} 
            className="w-24 h-24 rounded-lg shadow-md object-cover hidden md:block" // Caché sur mobile car le lecteur a déjà la cover
          />
      ...
      {/* --- LISTE : TITRES À VENIR (SANS LECTEURS) --- */}
      <div className="flex-1 overflow-hidden flex flex-col">
        <h4 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-3 flex-shrink-0">
            Up Next (Adapted)
        </h4>
        
        <div className="overflow-y-auto pr-2 space-y-3 flex-1">
          {upcomingTracks.map((track, index) => (
            <div key={`${track.id}-${index}`} className="bg-gray-800/50 hover:bg-gray-800 p-3 rounded-lg transition-colors group flex items-center justify-between">
              
              {/* Infos du morceau */}
              <div className="flex items-center gap-3 overflow-hidden">
                <span className="text-gray-500 font-mono text-sm w-4 flex-shrink-0">{index + 1}</span>
                <img src={track.coverUrl} alt="mini cover" className="w-10 h-10 rounded object-cover opacity-80 flex-shrink-0" />
              ...
              {/* Badges et Durée (à droite) */}
              <div className="flex items-center gap-3 flex-shrink-0 ml-2">
                <span className="text-[10px] px-2 py-0.5 rounded-full bg-gray-700 text-gray-300 border border-gray-600 hidden sm:inline-block">
                  {track.type}
                </span>
                <span className="text-xs text-gray-500 flex items-center gap-1 w-10 justify-end">
                  <Clock size={10} /> {Math.floor(track.duration / 60)}:{(track.duration % 60).toString().padStart(2, '0')}
                </span>
              </div>
